...
with st.container(border=True):
    ...
    def generate_pdf_report():
        ...
        if not section3_chart_html:
            sample_agents = [("TIGER SK하이닉스단일종목레버리지", 1250), ("KODEX SK하이닉스단일종목레버리지", 1180), ("TIGER 미국우주테크", 1020), ("SOL AI반도체TOP2플러스", 950), ("KODEX 삼성전자단일종목레버리지", 680)]
        ...
        # ----------------------------------------------------------------------
        marketing_news_text = st.session_state.get('sec5_marketing_news', "삼성자산운용 KODEX는 고객 가치 중심의 월배당 ETF 마케팅 캠페인을 활발히 전개 중입니다.")
        ai_insight_text = st.session_state.get('ai_insight_text', "매크로 불확실성 구간에서는 성장주와 인컴형 자산을 융합한 바벨 전략이 가장 유효합니다.")

        kodex_press_dynamic_html = ""
        kodex_list = st.session_state.get('kodex_weekly_marketing_list', [])
        if kodex_list:
            try:
