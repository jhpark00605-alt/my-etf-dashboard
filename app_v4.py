    ...
    def generate_pdf_report():
        ...
        if not datalab_box_chart_html:
            ...
            for d_date, d_val, d_bar in sample_dl:
                ...
                """
# ----------------------------------------------------------------------
# 🕒 [시간 설정] 서버 시간을 한국 표준시(KST, UTC+9)로 보정 (에러 원인 해결)
# ----------------------------------------------------------------------
from datetime import datetime, timedelta
...
# ----------------------------------------------------------------------
# 📑 [파이썬 실행 구역] HTML 주입이 정상 종료된 후, 바깥 공간에서 PDF를 컴파일합니다.
# ----------------------------------------------------------------------
pdf_buffer = BytesIO()
pisa_status = pisa.CreatePDF(html_string, dest=pdf_buffer, encoding='utf-8')

if pisa_status.err:
    st.error("PDF 컴파일 에러 발생")
else:
    pdf_buffer.seek(0)
    pdf_data = pdf_buffer.getvalue()

    # ----------------------------------------------------------------------
    # 📥 Streamlit 다운로드 버튼 렌더링 구역
    # ----------------------------------------------------------------------
    try:
        if pdf_data:
            from datetime import datetime, timedelta
